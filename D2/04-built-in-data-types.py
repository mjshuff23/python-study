...
#
### Lists ################
# - Lists in Python are very similar to arrays in JavaScript. They are 
#    typically used to store a sequences of items that are all the same type 
#    (homogeneous).
#
# - Lists are mutable, meaning they can be changed. Primarily this means 
#    sorting, as well as adding and removing items.
#
...
### Tuples #################
# - Tuples are very similar to lists in Python. The primary difference is that 
#    they are immutable, which means the tuple cannot be changed after creation.
#
...
# - The tuple built-in may be used to convert another iterable (like string or
#    list) to a tuple.
tuple('abc') # ('a', 'b', 'c')
tuple([1, 2, 3]) # (1, 2, 3)
...
#    - step - optional (1 if not supplied) - the difference between each number 
#                                             in the sequence
range(5) # [0, 1, 2, 3, 4]
range(1, 5) # [1, 2, 3, 4]
...
### Dictionaries ####################
# - A dictionary is a mappable collection where a hashable value (a.k.a. hash) 
#    is used as a key to reference an object stored in the dictionary. 
#    Dictionaries often hold arbitrary objects which are quite different 
#    (heterogenous). Dictionaries are mutable and may be changed at any time.
#
# - Often a dictionary is declared with curly braces or the dict built-in.
a = {'one': 1, 'two': 2, 'three': 3}
b = dict(one=1, two=2, three=3)

# - Sometimes you will have tuples you want to put into a dictionary. That is 
#    also done with the dict built-in. Here's an example using a list of tuples.
c = dict([('two', 2), ('one', 1), ('three', 3)])

# - A useful benefit of dictionaries in Python is that it doesn't matter how 
#    the dictionary is defined, if the keys and values are the same, the 
#    dictionaries are considered equal. Using a, b and c from above...
print(a == b)        # => True
print(a == c)        # => True
print(b == c)        # => True
print(a == b == c)   # => True
...
#    objects need to hashable. And they will always be unique, meaning
#    duplicate items are automatically dropped from the set.
# 
...
#     difference
#
# - A standard set is mutable. Python has an immutable variation called 
#    `frozenset`.
#
# - Sets can be created by putting comma-separated values inside braces.
school_bag = {'book', 'paper', 'pencil', 'pencil', 'book', 'book', 'eraser'}
print(school_bag)
